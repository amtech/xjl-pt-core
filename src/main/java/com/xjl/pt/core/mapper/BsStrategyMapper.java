...
public interface BsStrategyMapper {
	...
	static final String UPDATE_FIELD = "strategy_state=#{strategyState},strategy_content=#{strategyContent},strategy_title=#{strategyTitle}," + XJLMapper.FIX_UPDATE_FIELD;
	 ...
	 * 通过id屏蔽攻略
	 */
	@Delete("update "+ TABLE_NAME +" set strategy_state='00' "+" where strategy_id=#{strategyId}")
	public void updateStateById(String strategyId);
	/**
	 * 通过用户id查找攻略数据
	 * @return
	 */
	@Select("select " + SELECT_ALL +" from " + TABLE_NAME + " where state='A' and create_user_id = #{createUserId}")
	public List<BsStrategy> selectByUserId(String userId);
}